    ...
    @forelse($posts as $post)
        <div class="flex p-4 border-b border-b-gray-400">
            <div class="mr-2 flex-shrink-0">
                <a href="{{route('profile.show',[$post->user])}}"><img src="{{$post->user->avatar}}"
                                                                       alt=""
            ...
            <div class="overflow-clip overflow-hidden ...">
                <a href="{{route('profile.show',[$post->user])}}"><h5 class="font-bold mb-4">
                        {{$post->user->name}}
                    </h5></a>
                <p class="text-sm">
                    {{$post->body}}</p>
                <div class="flex">
                    <form method="POST" action="{{route('post.like',$post->id)}}">
                        ...
                        <div class="flex items-center mr-4 {{$post->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500'}}">
                            ...
                            <button type="submit" class="text-xs text-gray-500 mr-3">{{$post->likes ?: 0}}</button>
                        </div>
                    </form>
                    <form method="POST" action="{{route('post.dislike',$post->id)}}">
                        @csrf
                        @method('DELETE')

                        <div class="flex
                        items-center {{$post->isDisLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500'}}">
                            <svg viewBox="0 0 20 20" class="mr-1 w-4">
                                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                    <g class="fill-current">
                                        <path
                            ...
                            <button type="submit" class="text-xs text-gray-500">{{$post->dislikes ?: 0}}</button>
                        </div>
                    </form>
                </div>
            </div>
